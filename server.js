  ...
  const rows = db.prepare(`
    SELECT e.id, e.bundle_number, e.created_at, e.completed_at, e.score, e.total, e.passed,
      (SELECT COUNT(*) FROM exam_questions eq
       WHERE eq.exam_id = e.id AND eq.user_answer IS NOT NULL) as answered
    FROM exams e
...
// Fetch an exam with its questions
...
app.post('/api/exams/:id/submit', (req, res) => {
  ...
  const rows = db.prepare(`
    SELECT eq.question_id, eq.position, q.answer AS correct, q.domain
    FROM exam_questions eq
    JOIN questions q ON q.id = eq.question_id
    WHERE eq.exam_id = ?
    ORDER BY eq.position
